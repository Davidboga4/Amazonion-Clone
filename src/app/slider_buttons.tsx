...
const SliderButtons = ({ goToPrevious, goToNext }: any) => {
    return (
        <div className="absolute top-0 h-62 w-full flex justify-between px-4 hidden sm:flex">
            <div role="button" aria-label="Previous Slide" className="flex items-center justify-center rounded-lg m-1 cursor-pointer active:bg-white active:opacity-20 active:ring-1" onClick={goToPrevious}>
                <ChevronLeft className="text-black" size={54} strokeWidth={1} />
            </div>
            <div role="button" aria-label="Next Slide" className="flex items-center justify-center rounded-lg m-1 cursor-pointer active:bg-white active:opacity-20 active:ring-1" onClick={goToNext}>
